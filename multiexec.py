...
def singelTarget(opts):
	...
	longest = (len(max(opts.target, key=len).strip()) + 5)
	...
	for opts.hostname in opts.target:
		...
		opts.screen = str('%s] %s' % (opts.totalhosts, opts.hostname)).ljust(longest)
...
def multiTarget(opts):
	...
	with open(opts.hostfile, 'r') as longhost:
		longest = (len(max(longhost, key=len).strip()) + 5)
...
class SshExec(threading.Thread):
	...
	def run(self):
		...
		if (self.Platcheck):
			...
			if (exitcode != 0):
				myprint("[%s/%s %s" % (Count(), self.screen, '- Platform excluded!'))
